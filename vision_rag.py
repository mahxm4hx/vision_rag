...
st.title("Trial Vision RAG Of Environ 🖼️")

# --- Permanent API Keys ---
# Replace these with your actual API keys
COHERE_API_KEY = "xQXRlNl37fIvTNxu1UtyrHQjbjbaalChL8s4cYfD"
GOOGLE_API_KEY = "AIzaSyBu-Eq1eVfJX9A1ZNLBCtUdNRKr_V5lrsg"
...
# Initialize clients with permanent API keys
try:
...
try:
    ...
    st.error("Please check your Google API key in the code.")
...
if co:
    ...
    if st.button("Load Sample Images", key="load_sample_button"):
     ...
     st.warning("API clients not properly initialized. Check your API keys.")
...
run_button = st.button("Run Vision RAG", key="main_run_button", 
                      disabled=not (co and genai_client and question and st.session_state.image_paths and st.session_state.doc_embeddings is not None and st.session_state.doc_embeddings.size > 0))
...
# Footer
st.markdown("---")
st.caption("Vision RAG with Cohere Embed-4 | Created by Maneesh, Jithin, Samanuay and Mahesh")